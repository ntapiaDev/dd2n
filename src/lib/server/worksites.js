...
export const get_worksites = async (rethinkdb) => {
    return r.table('worksites').orderBy(r.asc('temporary'),r.asc('rarity'), r.asc('defense'), r.asc('name')).run(rethinkdb);
}

export const get_worksites_by_group = async (rethinkdb) => {
    return r.table('worksites').group("parent").orderBy(r.asc('temporary'), r.asc('rarity'), r.asc('defense'), r.asc('name')).run(rethinkdb);
}
