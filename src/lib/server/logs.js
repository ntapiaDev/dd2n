...
export const add_log = async (game_id, coordinate, player, action, log, gender, rethinkdb) => {
    return r.table('logs').insert({ game_id, coordinate, player, action, log, gender, 'date': Date.now() }).run(rethinkdb);
}
