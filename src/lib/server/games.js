...
export const add_one_day = (game_id, rethinkdb) => {
    return r.table('games').get( game_id ).update({ day: r.row('day').add(1) }).run(rethinkdb);
}

export const add_unique = (game_id, uniques, rethinkdb) => {
...
export const delete_game = (game_id, rethinkdb) => {
    return r.table('games').get(game_id).delete().run(rethinkdb);
}

export const get_game_by_id = (game_id, rethinkdb) => {
    return r.table('games').get(game_id).run(rethinkdb);
}

export const get_games = (rethinkdb) => {
