...
import { add_game, add_user_to_game, delete_game, get_games, get_game_by_id, remove_user_to_game } from "$lib/server/games"
import { add_game_to_user, remove_game_to_user, remove_game_to_users } from "$lib/server/users";
...
const deleteGame = async ({ locals, request }) => {
    ...
    await remove_game_to_users(game_id, locals.rethinkdb);
...
const joinGame = async ({ locals, request }) => {
    ...
    if (locals.user.game_id === game_id) {
        await remove_user_to_game(game_id, locals.user.username, locals.rethinkdb);
        await remove_user_from_location(game_id, locals.user.username, locals.user.location, locals.rethinkdb);
        await remove_game_to_user(locals.user.username, locals.rethinkdb);
        ...
        await add_game_to_user(game_id, locals.user.username, encampment, locals.rethinkdb);
